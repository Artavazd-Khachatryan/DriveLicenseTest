...
`GoogleService-Info.plist` is bundled via the **iosApp** Xcode target. The native Firebase iOS SDK is linked through Swift Package Manager (`FirebaseCore`, `FirebaseCrashlytics` in `iosApp.xcodeproj`).
...
Initialization runs in `App.kt` via `CrashReporting.initialize()`.
